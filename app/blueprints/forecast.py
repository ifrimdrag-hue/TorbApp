...
# Upload jobs — fiecare upload async primește un UUID
_upload_jobs: dict = {}
...
def _run_upload_job(job_id: str, tip: str, fisier_orig: str, dest_path: str):
    """Rulează importul într-un thread daemon și actualizează _upload_jobs."""
    ...
    try:
        ...
        # import_stoc.py:        "→ Stoc importat: 699 rânduri"
        # import_vanzari_erp.py: "→ Inserate: 135,420 | Duplicate..."
        randuri = None
...
def api_upload_status(job_id):
    ...
    if job is None:
        return jsonify({
            'ok': True,
            'status': 'error',
            'mesaj': 'Serverul a fost repornit în timpul importului. Verificați istoricul sau reimportați fișierul.',
            'randuri': None,
        })
