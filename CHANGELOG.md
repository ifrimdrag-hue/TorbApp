...
- **EAN fallback safety** — both sides of the barcode comparison go through `_norm_ean()` (strips whitespace, a leading Excel apostrophe, and a trailing `.0` from float conversion), so a barcode stored untidily on Shopify still matches. When two report SKUs share one EAN the quantity is ambiguous, so the fallback is **disabled** for that EAN (summing would have pushed an inflated stock) and a warning is raised — duplicate EANs are a known catalog issue, see BACKLOG item 14.
...
- Files: `app/automations/stocuri_shopify/orchestrator.py`, `app/automations/stocuri_shopify/api_client.py`, `app/static/js/stocuri.js`, `tests/test_shopify_orchestrator.py` (new). Tests: 347 passing (+9).
