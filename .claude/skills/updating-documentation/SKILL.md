...
- **Summaries in CHANGELOG, state in STATUS.** The delivery write-up lives in `CHANGELOG.md` (one dated entry per feature); `STATUS.md` gets only the state flip (`[x]` / date) + next step — never a prose delivery summary.
...
|-----|------|--------------------------------|
| `CHANGELOG.md` | **Canonical home for implementation/delivery summaries** — dated log of every notable change (Keep-a-Changelog: `## [Unreleased]` → dated `### <Feature> (YYYY-MM-DD)` sections, bullets, closing "Documented in …" refs) | **Always** — add one dated entry per delivered feature (the full write-up lives here) |
| `context/STATUS.md` | Live **forward** tracker: open decisions, 90-day actions, in progress / blocked, "Next immediate step" — **not** delivery summaries (those go to CHANGELOG) | **Always** — flip the state marker + next step (a one-line state change, not a delivery write-up) |
...
4. **Edit surgically.** Update each affected section in place, matching its style. Always add the **CHANGELOG entry** (the delivery summary) and flip the **STATUS** state (one line + next step, no prose summary). Tick delivered BACKLOG items and annotate blocked ones. Keep every fact in its one canonical doc; cross-reference elsewhere.
...
- Writing a delivery summary into STATUS (or any doc but CHANGELOG) → the write-up belongs in `CHANGELOG.md`; STATUS only flips state.
