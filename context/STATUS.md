...
- **2026-07-04 — Model de forecast client × articol livrat, în spatele unui toggle (implicit OFF).**
  `app/forecast/pair_engine.py`: prognoză pe fiecare pereche client × articol (nu medie pe SKU) — fereastră adaptivă per pereche (§4.1), medie cu zerouri pentru lunile fără vânzări (§4.2), index sezonier plafonat `[0.2, 5.0]` cu prag de activare 24 luni istoric (§4.3), status `SUSPECT` delistare cu prag adaptiv `max(180 zile, 3× interval mediu de comandă)` (§5.1), agregare la nivel de articol (§2). Rezolvă direct backlog **B4** (SKU-uri delistate/în declin comandate în continuare). Tabel nou `forecast_config` (migrația `0017`) + `app/forecast/config.py` + card „Parametri forecast" pe `/forecast/setari`. Formulă comandă parțial (§8): `split_with_safety` — stoc de siguranță = coeficient × prognoză lunară, rotunjire la bax; **MOQ nerezolvat** (decizie 6). Cuplat prin `?model=nou` (`build_suggestion`, `forecast_stoc_extended`) — implicit rămâne modelul vechi (`model=actual`) neschimbat. `?compare=1` afișează ambele modele alăturat (coloane `_nou`) pentru validare owner înainte de schimbarea implicitului. UI: toggle model, badge „Suspect delistare", marcaj sezonalitate „fără ajustare (<24 luni)", popover transparență sugestie. Itemii §4.4, §5 (ciclu complet DELISTAT/REACTIVAT + confirmare), §6 (ramp-up listare nouă), §8 (prag MOQ), §10 (recalcul batch vs. on-demand) rămân blocați pe decizii owner — vezi `docs/decision.html` itemii 5–10. 201 teste trec, ruff curat. Detalii: `app/forecast/README.md` §„Client × article model".
