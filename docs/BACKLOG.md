...
**Spec-completion engine pieces (2026-07-04)** — the low-risk, fully-specified parts of the owner brief/spec landed behind the same toggle after the owner docs resolved decisions 1–10: **neutral months** (Brief §4.1 level-1 multi-client heuristic, item 5), **INACTIV 6-month cut** (§7), **DELISTAT auto-confirm label** (§5.2, item 7 partial), **MOQ floor mechanism** (§8, item 6 — inert, no data), and a **daily stock-snapshot ETL** (`etl/snapshot_stoc.py`) to accrue OOS history. Still deferred (need judgment/data/UI): new-listing ramp-up (§6, item 8), manual delisting-confirmation UI + exception report (§10), the "an curent vs. an trecut" view (§9), MOQ data, and F2/F3/F4 (order lifecycle, documents, batch stock). RO/Export split — owner confirmed **keep**. Open owner questions now: `app/templates/decision_torb.html` items 6/9/11–14. Plan + spec digest: `docs/plans/2026-07-04-forecast-spec-completion.md`.
